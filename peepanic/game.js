...
function regulate() {
    ...
    if (bladder >= 100) {
        ...
        if (!pantPissed) {
            ...
            for (let i = 0; i < 5; i++) { // gradual piss
            ...
            peeMessage.textContent = "";
            document.getElementById("gameover-text").textContent = `💥 Your bladder exploded! Final Score: ${score - 5}`;
    ...
    if (water <= 5) {
        ...
        document.getElementById("gameover-text").textContent = `💥 You died of dehydration! Final Score: ${score - 5}`;
    ...
    if (salt <= 5) {
        ...
        document.getElementById("gameover-text").textContent = `💥 You died of hyponatremia! Final Score: ${score - 5}`;
    ...
    if (salt >= 95) {
        ...
        document.getElementById("gameover-text").textContent = `💥 You died of hypernatremia! Final Score: ${score - 5}`;
    ...
    if (droughtActive) {
        ...
        if (Math.random() < 1 / 3) { // 1 in 3 chance that 0-10 seconds will be added 20 seconds drought in worst case
            droughtDuration += Math.floor(Math.random() * 10); // up to +20
        }

        drinkBtn.classList.add("disabled");
        const droughtMessages = [
            "No water taps around 💧",
...
function tickTimer() {
    if (timeLeft > 0) {
        ...
        if (score >= 100) {
            ...
            document.getElementById("gameover-text").innerHTML = `🔥 You conquered the chaos! 💦<br>Final Score: ${score}<br><strong>Osmoregulation Master!</strong><br>Dare to beat perfection?`;

        } else {
            document.getElementById("gameover-text").textContent = `Time's up! Final Score: ${score}, try get 100 or more!`;
        }
        document.getElementById("gameover").style.display = "flex";
        clearInterval(timerInterval);
        clearInterval(regulationInterval);
...
setInterval(() => {
    if (!goldenToiletActive && Math.random() < 0.25) { // 0.25 25% chance every interval
        spawnGoldenToilet();
    }
}, 5000);
